...
int msx_test(void)
{
	uint8_t	p_i8[BLOCK_BYTE] = {0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77};
	...
	uint8_t exp[BLOCK_BYTE] = {0xfa, 0x39, 0x65, 0x32, 0xaf, 0xbe, 0xe7, 0x94};
	uint32_t rk[RK_NUM_F*ROUND];
	...
	if(memcmp(c_i8, exp, BLOCK_BYTE) != 0)
	{
		...
		printf("EXP : ");
		for(uint32_t i = 0; i < BLOCK_BYTE; i++) printf("%02x",exp[i]);
		printf("\nACT : ");
		for(uint32_t i = 0; i < BLOCK_BYTE; i++) printf("%02x",c_i8[i]);
		printf("\n");
		return -1;
	}
	...
	if(memcmp(d_i8, p_i8, BLOCK_BYTE) != 0)
	{
		...
		printf("Dec : ");
		for(uint32_t i = 0; i < BLOCK_BYTE; i++) printf("%02x",d_i8[i]);
		printf("\n");
		return -1;
	}
